...
import { Activity, List, Grid3X3, ArrowUpDown, Filter } from 'lucide-react'
  ...
  return (
    <div className="flex-1 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <Link href={`/dashboard/infants/${params.id}`} className="flex items-center text-blue-600 hover:text-blue-800 mb-2">
                  ...
                  return (
                    <div key={infantMilestone._id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="text-lg font-medium text-gray-900">{milestone.name}</h3>
                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(infantMilestone.status)}`}>
                          {infantMilestone.status}
                        </span>
                      </div>
                      ...
                      <div className="mt-3">
                        <label className="block text-sm font-medium text-gray-700 mb-1">{t('update')}</label>
                        <select
                          value={infantMilestone.status}
                          onChange={(e) => handleStatusChange(milestone._id, e.target.value)}
                          className="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
